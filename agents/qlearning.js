...
// API:
//  new QLearningAgent({states, actions, alpha, gamma, epsilon})
//  init(states, actions)
//  chooseAction(state)
...
export default class QLearningAgent {
  ...
  // Q-learning update: off-policy using max over next actions
  update(s, a, r, sNext, done=false){
    this.ensureState(s);
    this.ensureState(sNext);
    const qsa = this.Q[s][a];
    const maxNext = done ? 0 : Math.max(...this.Q[sNext]);
